...
class MjpegView : SurfaceView, SurfaceHolder.Callback {
    ...
    internal inner class MjpegViewThread(private val mSurfaceHolder: SurfaceHolder) {
        ...
        private fun initThread() {
            join()
            if (thread != null) join()
            thread = MjpegRenderThread(mSurfaceHolder)
        }
        ...
        override fun run() {
            ...
            while (mRun) {
                ...
                try {
                    ...
                    if (canvas != null && mBitmap != null) DrawToCanvas(canvas, destRect)
                } catch (e: IOException) {
                    mRun = false
                } finally {
                    if (canvas != null) {
...
//            opts.inScaled = true;
        ...
        private fun DrawToCanvas(canvas: Canvas, destRect: Rect) {
            ...
            synchronized(mSurfaceHolder) {
                ...
                synchronized(this) { if (mBitmap != null) canvas.drawBitmap(mBitmap!!, null, destRect, null) }
